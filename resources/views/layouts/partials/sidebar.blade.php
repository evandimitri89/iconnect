<aside :class="collapsed ? 'w-16' : 'w-60'"
  ...
  <div class="px-3 py-2 mt-2">
    ...
    <ul class="space-y-1">
      <li class="group relative">
        <a href="{{ route('dashboard') }}" :class="collapsed ? 'justify-center' : 'justify-start'"
  ...
  <div class="border-t border-blue-500 p-3 mt-auto relative" x-data="{ open: false }">
    <button @click="open = !open"
      class="flex items-center gap-3 w-full px-2 py-2 rounded-lg transition duration-200
        text-white/90 hover:bg-[#2978BD]/60"
      :class="collapsed ? 'justify-center' : 'justify-start'">


      <img src="{{ asset('img/profile.png') }}" :class="collapsed ? 'w-6 h-6' : 'w-8 h-8'"
        class="rounded-full object-cover border border-white/50 shadow-sm" alt="profile">
    ...
    <!-- DROPDOWN: posizione disesuaikan jika collapsed -->
    <div x-show="open" x-transition @click.away="open = false" x-cloak
      :class="collapsed ? 'absolute bottom-20 left-full ml-2 w-48' : 'absolute bottom-16 left-2 w-[calc(100%-1rem)]'"
